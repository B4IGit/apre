...
// Test the sales report API
describe("Apre Sales Report API - Sales by Product - Customer", () => {
  beforeEach(() => {
    mongo.mockClear();
  });

  // Test the sales/sales-by-product/:product endpoint
  it("should fetch sales data for a specific product, grouped by customer", async () => {
    mongo.mockImplementation(async (callback) => {
      // Mock the MongoDB collection
      const db = {
        collection: jest.fn().mockReturnThis(),
    ...
    const response = await request(app).get(
      "/api/reports/sales/sales-by-product-customer/4k TV",
    ); // Send a GET request to the sales/sales-by-product/:product endpoint
    expect(response.status).toBe(200); // Expect a 200 status code
  ...
  it("should return 200 and an empty array if no sales data is found for the product", async () => {
    // Mock the MongoDB implementation
    mongo.mockImplementation(async (callback) => {
      const db = {
        collection: jest.fn().mockReturnThis(),
    ...
    // Make a request to the endpoint
    const response = await request(app).get(
      "/api/reports/sales/sales-by-product-customer/unknown-product",
    );
  ...
  it("should return 404 for an invalid endpoint", async () => {
    // Make a request to an invalid endpoint
    const response = await request(app).get(
      "/api/reports/sales/invalid-product",
    );
